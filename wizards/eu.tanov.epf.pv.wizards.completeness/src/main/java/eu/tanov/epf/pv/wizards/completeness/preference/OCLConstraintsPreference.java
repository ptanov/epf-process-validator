...
import eu.tanov.epf.pv.wizards.completeness.CompletenessActivator;
...
public class OCLConstraintsPreference extends FieldEditorPreferencePage implements IWorkbenchPreferencePage {
	...
	private static final String CATEGORY = "eu.tanov.epf.pv.validators.category.default/Wizards/Completeness";
	//TODO not public!
	public static final IPreferenceStore PREFERENCE_STORE = CompletenessActivator.getDefault().getPreferenceStore();
	...
	public void createFieldEditors() {
		...
		final OCLConstraintsService service = CompletenessActivator.getDefault().getService(OCLConstraintsService.class);
	...
	public static void registerOCLContent() {
		...
		final OCLConstraintsDefinition definitionFromPreferences = CompletenessActivator.getDefault().getDefinitionFromPreferences();
		if (definitionFromPreferences != null) {
			service.removeConstraintsDefinition(definitionFromPreferences);
		}
		// register new:
		final OCLConstraintsDefinition definition = new OCLConstraintsDefinition(CompletenessActivator.PLUGIN_ID, NAME_OCL_CONTENT,
				CATEGORY, false, ConstraintSeverity.ERROR, oclContent, "Completeness %s violated on {0}");
		service.registerConstraintsDefinition(definition);
		CompletenessActivator.getDefault().setDefinitionFromPreferences(definition);
	}
