...
public class StandardHelper {
	...
	/**
	 * (#42) If task contains work products as mandatory input and task is selected in Standard - automatically add its work
	 * products (that are mandatory inputs) to Standard
	 * 
	 * Order of automatically added work products is not guaranteed (HashSet is used)
	 */
	public static void updateWorkProducts(CustomCategory standard) {
		final HashSet<WorkProduct> directWorkProducts = new HashSet<WorkProduct>(
				TypeFilteredContentElementOrderList.<WorkProduct> toFilteredList(standard,
						UmaPackage.eINSTANCE.getCustomCategory_CategorizedElements(), WorkProduct.class));

		final List<Task> tasks = TypeFilteredContentElementOrderList.<Task> toFilteredList(standard,
		...
		standard.getCategorizedElements().addAll(workProductsToAdd);
	}
